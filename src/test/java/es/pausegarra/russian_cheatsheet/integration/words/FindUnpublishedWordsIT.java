...
@QuarkusTest
public class FindUnpublishedWordsIT extends IntegrationTest {

  @Test
  @TestSecurity(user = "user", roles = "words#publish")
  public void shouldFindUnpublishedWordsByCriteriaWithDefaultValues() {
  ...
  public void shouldChangePaginationParameters() {
    List<WordEntity> words = List.of(WordMother.random().id(null).publishedAt(null).build(), WordMother.random().id(null).publishedAt(null).build());
  ...
  public void shouldSearch() {
    List<WordEntity> words = List.of(
      WordMother.random().id(null).russian("search").publishedAt(null).build(),
      WordMother.random().id(null).russian("hidden").publishedAt(null).build()
    );
    persist(WordModel.fromEntity(words.getFirst()));
    persist(WordModel.fromEntity(words.getLast()));
